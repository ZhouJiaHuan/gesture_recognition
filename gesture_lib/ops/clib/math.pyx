...
def cross_point(list point11,
    ...
    cdef list point = [-1, -1]
    cdef int w, h
    cdef float x11, y11, x12, y12, x21, y21, x22, y22
    w, h = img_size
    x11, y11 = point11
    x12, y12 = point12
    x21, y21 = point21
...
def cross_point_in(list point11,
                   list point12,
                   list point21,
                   list point22,
                   list img_size):
...
def computeP3withD(list point1,
                   list point2,
                   float d):
    '''
    ...
    '''
    cdef int dim
    cdef float x1, y1, z1, x2, y2, z2, k, x, y, z
    dim = len(point1)
    if dim == 2:
        x1, y1 = point1
        x2, y2 = point2
        k = d / sqrtf((x2-x1)**2 + (y2-y1)**2)
        x = k * (x2 - x1) + x1
        y = k * (y2 - y1) + y1
        return (x, y)
    elif dim == 3:
        ...
        x1, y1, z1 = point1
        x2, y2, z2 = point2
        k = d / sqrtf((x2-x1)**2 + (y2-y1)**2 + (z2-z1)**2)
        x = k * (x2 - x1) + x1
        y = k * (y2 - y1) + y1
        z = k * (z2 - z1) + z1
        return (x, y, z)
